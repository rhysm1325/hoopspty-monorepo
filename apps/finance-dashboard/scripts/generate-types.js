...
// TypeScript type generation script for AUSA Finance Dashboard

const fs = require('fs')
const path = require('path')

/**
 * Generate comprehensive TypeScript types based on our database schema
 */
function generateDatabaseTypes() {
  const typesContent = `// Database types for AUSA Finance Dashboard
...
// Enums from database
export type UserRole = 'owner' | 'finance' | 'operations' | 'sales' | 'marketing'
export type RevenueStream = 'tours' | 'dr-dish' | 'marketing' | 'other'
export type SyncStatus = 'idle' | 'running' | 'completed' | 'error' | 'cancelled' | 'partial'
...
// View types
export type UserProfileWithStats = Database["public"]["Views"]["user_profiles_with_stats"]["Row"]
export type RevenueByWeek = Database["public"]["Views"]["fact_revenue_by_week"]["Row"]
export type CashPosition = Database["public"]["Views"]["fact_cash_position"]["Row"]
export type SyncStatusDashboard = Database["public"]["Views"]["sync_status_dashboard"]["Row"]
...
function main() {
  ...
  try {
    ...
    console.log('   - All table definitions with Row/Insert/Update types')
    console.log('   - View definitions for analytics queries')
    console.log('   - Function signatures for database functions')
    console.log('   - Enum types for type safety')
    console.log('   - Helper types for common operations')

    console.log('\n💡 To use these types in your code:')
    console.log(
      '   import type { Profile, SyncCheckpoint, RevenueByWeek } from "@/types/database-generated"'
    )
  } catch (error) {
